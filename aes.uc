...
export function AES_ExpandKey(key) {
  let kl = length(key), ks, Rcon = 1;
  switch (kl) {
    case 16: ks = 16 * (10 + 1); break;
    case 24: ks = 16 * (12 + 1); break;
    case 32: ks = 16 * (14 + 1); break;
    default: 
      print("AES_ExpandKey: Only key lengths of 16, 24 or 32 bytes allowed!\n");
  }
  for(let i = kl; i < ks; i += 4) {
    let temp = slice(key, i - 4, i);
    if (i % kl == 0) {
...
export function AES_Encrypt(block, key) {
  let l = length(key);
  ...
  for(i = 16; i < l - 16; i += 16) {
    ...
    AES_AddRoundKey(block, slice(key, i, i + 16));
  }
  AES_SubBytes(block, AES_Sbox);
  AES_ShiftRows(block, AES_ShiftRowTab);
  AES_AddRoundKey(block, slice(key, i, l));
  return block;
};
...
export function AES_Decrypt(block, key) {
  let l = length(key);
  AES_AddRoundKey(block, slice(key, l - 16, l));
